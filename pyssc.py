...
def Sij(z_arr, window1_arr, window2_arr, cosmo_params=default_cosmo_params):

    # Assert everything as the good type and shape, and find number of redshifts, bins etc
    zz   = np.asarray(z_arr)
    win1 = np.asarray(window1_arr)
    win2 = np.asarray(window2_arr)
    
    assert zz.ndim==1, 'z_arr must be a 1-dimensional array'
    assert win1.ndim==2, 'window1_arr must be a 2-dimensional array'
    assert win2.ndim==2, 'window2_arr must be a 2-dimensional array'
    
    nz = len(zz)
    nbin1 = win1.shape[0]
    nbin2 = win2.shape[0]
    assert win1.shape[1]==nz, 'window1_arr must have shape (nbins,nz)'
    assert win2.shape[1]==nz, 'window1_arr must have shape (nbins,nz)'
