...
##### Sij_AngPow_fullsky #####
def Sij_AngPow_fullsky(z_arr,windows,cosmo_params=default_cosmo_params,precision=10,cosmo_Class=None):
    """
    Routine to compute the Sij matrix with general window functions given as tables
    example : weak lensing, or galaxy clustering with redshift errors
    ...
    dX depends on the convention used to define the window functions : Cl(i,j) = int dX window(i,z) window(j,z) P(k=(ell+1/2)/r,z)
    Convention 0 (only implemented yet) : dX = dV = dV/dz dz = r^2(z) dr/dz dz. Used in Lacasa & Grain 2019.
    """
...
##### Sij_AngPow_psky #####
def Sij_AngPow_psky(z_arr,windows,clmask=None,mask=None,cosmo_params=default_cosmo_params,cosmo_Class=None,var_tol=0.05,verbose=False,debug=False): #Lmax
    """
    Sij_AngPow_psky
    Routine to compute the Sij matrix in partial sky
    ...
    Output : Sij matrix (shape: nbins x nbins)
    Options :
    - var_tol : precision setting to decide where to cut the sum over multipoles. Final precision on Sij should be better than var_tol. Default is 5%.

    Equation used : Sij = sum_ell (2ell+1) C(ell,i,j) C(ell,mask) /(4pi*fsky)^2
    where C(ell,i,j) = 2/pi \int kk^2 dkk P(kk) U(i;kk,ell)/Inorm(i) U(j;kk,ell)/Inorm(j) is computed by AngPow
    with Inorm(i) = int dX window(i,z)^2  and U(i;kk,ell) = int dX window(i,z)^2 growth(z) j_ell(kk*r)
    """
